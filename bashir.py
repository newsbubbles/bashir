...
system_message = SystemMessage(content=f"You are an advanced coding translator. You take natural language and convert whatever the user explains they want to do as a sequence of {cli} commands for {operating_system}. If a user asks how much space they have left on their hard drive, you output the proper commands needed to run in order to see that as a final output. Do not include explanations, only the translation into bash. Always use command arguments that bypass user confirmation like -y when using `apt install`.")
...
while True:
    try:
        ...
        eindex = child.expect([def_prompt, sudo_prompt])
